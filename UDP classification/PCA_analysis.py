...
def classifiers(data, n_PCA, save_dir):
    ...
    dataset_path = "/home/det_tesi/sgarofalo/PCA_analysis/train_dataset/dataset.csv"
    ...
    t = "GNB Learning Curve"
    plot_learning_curve(GNB, t, X, y, ylim=(0.0, 1.10), cv=cv, n_jobs=-1)
    save_photo(save_dir, t, str(n_PCA)+" PCA")
    '''
    # #############################################################################
    ...
    max_depth = [None, 8, 16]
    max_features = [2] + [x for x in range(2, n_PCA, 3)]
    ...
    # Test Set
    # #############################################################################
    dataset = pd.read_csv("/home/det_tesi/sgarofalo/PCA_analysis/test_dataset/dataset.csv")
    X = dataset.iloc[:, :-1]
    y = dataset.iloc[:, -1:]
    y[y.label == 'RTP'] = 1
    y[y.label == 'Not RTP'] = 0
...
data = {}
train_dir = "/home/det_tesi/sgarofalo/PCA_analysis/train_dataset"
test_dir = "/home/det_tesi/sgarofalo/PCA_analysis/test_dataset"
save_dir = "/home/det_tesi/sgarofalo/PCA_analysis"
pm = pcap_manager('1s')
#pm.merge_pcap(train_dir)
#pm.merge_pcap(test_dir)
for n_PCA in range(2, 15):
    print("Classifing with n_PCA = " + str(n_PCA))
    classifiers(data, n_PCA, save_dir)
    print()
